  ...
  return(
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"flex-end",zIndex:9999}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:"28px 28px 0 0",padding:"0 20px 40px",width:"100%",maxHeight:"88vh",overflowY:"auto",boxSizing:"border-box",maxWidth:480,margin:"0 auto"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}><span style={{fontWeight:800,fontSize:19}}>{title}</span><button onClick={onClose} style={{background:"none",border:"none",fontSize:18}}>✕</button></div>
        {children}
      </div>
    </div>
  );
...
function ProdsPage({products,setProducts,vendors,factories,user}){
  ...
  const addBom=()=>{
    if(!br.mat||!br.amt||!br.vid) return alert("항목을 입력하세요");
  ...
  return(
    <div style={{padding:"20px 16px 80px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}><div style={{fontSize:20,fontWeight:900}}>상품 관리</div><Btn ch="+ 추가" sz="s" onClick={()=>{setF({name:"",colors:[],colorBom:{},factoryId:""});setStep(0);setSheet(true);}}/></div>
      {products.map(p=><Card key={p.id} onClick={()=>{setF(p);setStep(0);setSheet(true);}}><div style={{fontWeight:800}}>{p.name}</div></Card>)}
      {sheet && <Sheet title="상품 정보" onClose={()=>setSheet(false)}>
        {step===0 ? <>
          ...
          <div style={{background:C.bg,padding:16,borderRadius:16,marginBottom:20}}>
            <div style={{fontWeight:800,marginBottom:12}}>[{selColor}] BOM 리스트</div>
...
// [발주하기] 로직 복구
function OrderPage({products,setOrders,vendors,factories,user}){
  const [selProd,setSelProd]=useState(null);
  const [selColor,setSelColor]=useState("");
  const [qty,setQty]=useState("");
  async function submit(){
    if(!selProd||!selColor||!qty) return alert("정보를 모두 입력하세요");
    const o={date:today(), status:"진행중", items:[{pid:selProd.id, color:selColor, qty:Number(qty)}], user_id:user.id};
    const r=await DB.insert(user.token,"orders",o);
    setOrders(prev=>[r[0],...prev]); alert("발주 완료!");
  }
  return(
    <div style={{padding:"20px 16px 80px"}}>
      <div style={{fontSize:20,fontWeight:900,marginBottom:24}}>발주 입력</div>
      <Card>
        <Field label="상품 선택"><select onChange={e=>setSelProd(products.find(p=>p.id===e.target.value))} style={{width:"100%",padding:12}}><option>상품 선택</option>{products.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></Field>
        <Field label="색상 선택"><select onChange={e=>setSelColor(e.target.value)} style={{width:"100%",padding:12}}><option>색상 선택</option>{selProd?.colors.map(c=><option key={c} value={c}>{c}</option>)}</select></Field>
        <Field label="발주 수량"><TxtInp val={qty} onChange={setQty} type="number" ph="장"/></Field>
        <Btn ch="발주 확정" full onClick={submit}/>
      </Card>
    </div>
  );
}
...
function ListPage({orders}){
  return(
    <div style={{padding:"20px 16px 80px"}}>
      ...
      {orders.map(o=><Card key={o.id}><div style={{display:'flex',justifyContent:'space-between'}}><div><div style={{fontWeight:800}}>{o.date} 발주</div><div style={{fontSize:12,color:C.sub,marginTop:4}}>{(o.items||[]).length}개 품목</div></div><Tag ch={o.status}/></div></Card>)}
    </div>
  );
}
...
function VendorPage({vendors,setVendors,user}){
  ...
  async function save(){
    const r=await DB.insert(user.token,"vendors",{...f,user_id:user.id});
    setVendors(prev=>[...prev,r[0]]); setSheet(false);
  }
  return(
    <div style={{padding:"20px 16px 80px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}><div style={{fontSize:20,fontWeight:900}}>거래처 관리</div><Btn ch="+ 추가" sz="s" onClick={()=>setSheet(true)}/></div>
      {vendors.map(v=><Card key={v.id} onClick={()=>{setF(v);setSheet(true);}}><div style={{fontWeight:800}}>{v.name} <Tag ch={v.type} c="#FF8A00"/></div><div style={{fontSize:12,color:C.sub,marginTop:4}}>📱 {v.tel}</div></Card>)}
...
export default function App(){
  ...
  if(screen==="loading") return <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700}}>WTMT 로딩 중...</div>;
  if(screen==="auth"||!user) return <AuthPage onLogin={handleLogin}/>;

  const pages={
    dash:<DashPage orders={orders} products={products}/>,
    order:<OrderPage products={products} setOrders={setOrders} vendors={vendors} factories={factories} user={user}/>,
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:C.fn,maxWidth:480,margin:"0 auto",position:"relative",boxShadow:"0 0 40px rgba(0,0,0,0.1)"}}>
      ...
      <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:480,background:"#fff",borderTop:`1px solid ${C.bdr}`,display:"flex",height:70,zIndex:50}}>
        {[
          {k:"dash",l:"대시보드"},{k:"order",l:"발주하기"},{k:"prods",l:"상품관리"},{k:"list",l:"발주리스트"},{k:"settings",l:"설정"}
        ].map(t=>(
