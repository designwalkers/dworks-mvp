...
function AuthPage({onLogin}){
  ...
  async function submit(){
    ...
    try{
      if(tab==="up"){
        const exist = await DB.findProfileByEmail(f.email);
        if(Array.isArray(exist) && exist.length > 0){
          setErr("이미 가입된 이메일입니다. 로그인해주세요.");
          setTab("in");
          return;
        }
        const r=await DB.signUp(
          f.email,
          f.pw,
          {
            company:f.company,
        ...
        if(r.error){
          setErr(
            r.error.message?.toLowerCase().includes("already")
              ? "이미 가입된 이메일입니다. 로그인해주세요."
              : r.error.message
          );
          return;
        }
        const identities = r.user?.identities || [];
        ...
        if(!r2.access_token){setErr("가입은 완료되었습니다. 로그인해주세요.");setTab("in");return;}
        try{
          await DB.updateProfile(r2.access_token, r2.user.id, {
            name: f.name,
            company: f.company,
            tel: f.tel,
            email: f.email
        ...
        let profile = null;
        try{
          const p = await DB.getProfile(r.access_token, r.user.id);
          profile = Array.isArray(p) && p.length > 0 ? p[0] : null;
        }catch(e){}
        const meta=r.user?.user_metadata||{};
        onLogin({token:r.access_token,id:r.user.id,name:profile?.name||meta.name||f.email.split("@")[0],company:profile?.company||meta.company||"",email:r.user.email,tel:profile?.tel||meta.tel||"",brand:meta.brand||"",position:meta.position||"",address:meta.address||""}, f.keepLoggedIn);
      }
