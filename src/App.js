...
// 🚀 Grid 삭제 및 Flexbox로 완벽하게 수정한 대시보드 🚀
...
      
      {/* 1. 상단 통계 카드 (절대 깨지지 않는 Flexbox 구조 적용) */}
      <div style={{display:"flex", width:"100%", marginBottom:14, border:`1px solid ${C.bdr}`, borderRadius:12, overflow:"hidden", background:C.bdr}}>
        {[{label:"오늘 발주",val:`${tO.length}건`,c:C.acc},{label:"미출고 발주",val:`${delayed.length}건`,c:"#8B5CF6"},{label:"이달 발주량",val:`${fmtN(mQ)}매`,c:C.ok}].map((s,i)=>(
          <div key={s.label} style={{flex:1, background:"#fff", padding:"12px 0", textAlign:"center", marginRight:i<2?1:0}}>
            <div style={{color:s.c, fontSize:20, fontWeight:900}}>{s.val}</div>
            <div style={{color:C.sub, fontSize:11, marginTop:3, fontWeight:600}}>{s.label}</div>
          </div>
        ))}
      </div>

      {/* 2. 지연 목록 카드 (테이블 레이아웃 깨짐 현상 수정) */}
...
        {delayed.length===0?<div style={{textAlign:"center",padding:"12px 0",color:C.sub,fontSize:12}}>지연 발주 없음 ✅</div>:<>
          {/* 테이블 헤더 */}
          <div style={{display:"flex", width:"100%", fontSize:11, fontWeight:600, color:C.sub, paddingBottom:6, borderBottom:`1px solid ${C.bdr}`, marginBottom:4}}>
            <div style={{flex:2}}>상품명</div>
            <div style={{width:60, textAlign:"center"}}>색상</div>
            <div style={{width:50, textAlign:"right"}}>수량</div>
            <div style={{width:50, textAlign:"center", marginLeft:8}}>상태</div>
          </div>
          {/* 테이블 데이터 */}
          {delayed.slice(0,5).flatMap(o=>(o.items||[]).map((it,j)=>{
            const p=products.find(x=>x.id===it.pid);
            return (
              <div key={`${o.id}-${j}`} style={{display:"flex", width:"100%", fontSize:12, padding:"8px 0", borderBottom:`1px solid ${C.bdr}`, alignItems:"center"}}>
                ...
                <div style={{width:50, display:"flex", justifyContent:"center", marginLeft:8}}>
                  <Tag ch="지연" c={C.warn}/>
                </div>
              </div>
            );
...
                    }} 
                    style={{width:50, padding:"4px 6px", border:`1px solid ${C.bdr}`, borderRadius:6, textAlign:"right", fontSize:13, fontWeight:700, color:C.acc, outline:"none"}} 
                  />
