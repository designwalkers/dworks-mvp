...
// [수리 1] 서버 에러(JWT 만료 등) 발생 시 가짜 데이터 처리를 막기 위해 전역 에러 처리
async function api(m,p,t,b){
  const r=await fetch(`${SB}${p}`,{method:m,headers:ah(t),body:b?JSON.stringify(b):undefined});
  const j=await r.json();
  if(j&&j.message&&j.message.includes("JWT expired")){
    alert("보안을 위해 로그인이 만료되었습니다. 다시 로그인해주세요.");
...
  updateUser:(t,meta)=>api("PUT","/auth/v1/user",t,{data:meta}),
  // [수리 2] 내 데이터만 확실히 가져오도록 필터(user_id) 및 select=* 추가
...
  function addItem(){if(!selProd||!selColor||!qty){alert("상품·색상·수량을 입력하세요");return;}const idx=items.findIndex(i=>i.pid===selProd.id&&i.color===selColor);if(idx>=0)setItems(p=>p.map((it,i)=>i===idx?{...it,qty:it.qty+Number(qty)}:it));else setItems(p=>[...p,{pid:selProd.id,color:selColor,qty:Number(qty)}]);setSelProd(null);setSelColor("");setQty("");setSearch("");}
  // [수리 3] 가짜 데이터 주입(uid) 삭제, 오류 시 에러창 표시
  async function submit(){if(!items.length){alert("발주 항목 추가");return;}const o={items,status:"진행중",date:today(),ts:new Date().toISOString()};try{if(user?.token){const r=await DB.insert(user.token,"orders",{...o,user_id:user.id});if(r.error||r.message){alert("오류: "+(r.message||"저장 실패"));return;}setOrders(p=>[...p,Array.isArray(r)?r[0]:r]);}else{alert("로그인이 필요합니다.");return;}}catch(e){alert("오류 발생");return;}try{localStorage.removeItem(DRAFT);}catch{}setStep(3);}
...
  function addBom(){if(!br.mat||!br.amt)return;if(editBomId){setF(p=>({...p,bom:p.bom.map(b=>b.id===editBomId?{...b,...br,amt:Number(br.amt)}:b)}));setEditBomId(null);}else setF(p=>({...p,bom:[...p.bom,{...br,id:uid(),amt:Number(br.amt)}]}));setBr({type:"",mat:"",amt:"",vid:""});setVenSearch("");}
  // [수리 4] 가짜 데이터 주입(uid) 삭제, 오류 시 에러창 표시
  async function save(){
    if(!f.name)return;
    try{if(f.id&&user?.token){const r=await DB.update(user.token,"products",f.id,{name:f.name,category:f.category,season:f.season,factory_id:f.factoryId,factory:f.factory,factory_tel:f.factoryTel,colors:f.colors,bom:f.bom});if(r.error||r.message){alert("오류: "+r.message);return;}setProducts(products.map(p=>p.id===f.id?f:p));}else if(user?.token){const r=await DB.insert(user.token,"products",{name:f.name,category:f.category,season:f.season,factory_id:f.factoryId,factory:f.factory,factory_tel:f.factoryTel,colors:f.colors,bom:f.bom,user_id:user.id});if(r.error||r.message){alert("오류: "+r.message);return;}setProducts(p=>[...p,Array.isArray(r)?{...r[0],factoryId:r[0].factory_id||"",factoryTel:r[0].factory_tel||"",colors:r[0].colors||[],bom:r[0].bom||[]}:r]);}}catch(e){alert("오류 발생");return;}
    setSheet(false);
  }
...
        <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:14}}>{CATS.map(cat=>{const act=f.category===cat;return<button key={cat} onClick={()=>sf("category")(cat)} style={{padding:"5px 11px",borderRadius:20,border:`1.5px solid ${act?(CAT_C[cat]||C.acc):C.bdr}`,background:act?(CAT_C[cat]||C.acc):"#fff",color:act?"#fff":C.sub2,fontWeight:600,fontSize:11,cursor:"pointer",fontFamily:C.fn}}>{cat}</button>;})}</div>
        <div style={{fontSize:12,fontWeight:600,color:C.sub,marginBottom:8}}>원단 정보</div>
        <FCard><FRow label="원단명"><FInp val={br.mat} onChange={v=>setBr(r=>({...r,mat:v}))} ph="예: 30수 면 싱글"/></FRow><FRow label="색상"><div style={{flex:1,display:"flex",flexWrap:"wrap",gap:4,justifyContent:"flex-end",alignItems:"center",padding:"4px 0"}}>{f.colors.map(c=><span key={c} style={{background:C.bg,borderRadius:20,padding:"2px 7px",fontSize:11,display:"flex",alignItems:"center",gap:3,border:`1px solid ${C.bdr}`}}>{c}<button onClick={()=>setF(p=>({...p,colors:p.colors.filter(x=>x!==c)}))} style={{background:"none",border:"none",color:C.sub,cursor:"pointer",fontSize:10,padding:0}}>✕</button></span>)}<div style={{display:"flex",gap:4}}><input value={ci} onChange={e=>setCi(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addColor()} placeholder="추가" style={{width:48,border:`1px solid ${C.bdr}`,borderRadius:6,padding:"2px 6px",fontSize:11,fontFamily:C.fn,outline:"none",textAlign:"center"}}/><button onClick={addColor} style={{background:C.acc,border:"none",color:"#fff",borderRadius:6,padding:"2px 8px",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:C.fn}}>+</button></div></div></FRow><FRow label="소요량"><FInp val={br.amt} onChange={v=>setBr(r=>({...r,amt:v}))} ph="0.0" type="number"/><span style={{color:C.sub,fontSize:11,flexShrink:0,marginLeft:4}}>yd</span></FRow><FRow label="단가" last><FInp val={br.price||""} onChange={v=>setBr(r=>({...r,price:v}))} ph="0" type="number"/><span style={{color:C.sub,fontSize:11,flexShrink:0,marginLeft:4}}>원</span></FRow></FCard>
...
  function openEdit(v){setF({...v});setEditId(v.id);setSheet(true);}
  // [수리 5] 가짜 데이터 주입(uid) 삭제, 오류 시 에러창 표시
  async function save(){if(!f.name)return;try{if(editId){if(user?.token){const r=await DB.update(user.token,"vendors",editId,{name:f.name,tel:f.tel,email:f.email,type:f.type});if(r.error||r.message){alert("오류: "+r.message);return;}setVendors(vv=>vv.map(v=>v.id===editId?{...v,...f}:v));}}else{if(user?.token){const r=await DB.insert(user.token,"vendors",{name:f.name,tel:f.tel,email:f.email,type:f.type,user_id:user.id});if(r.error||r.message){alert("오류: "+r.message);return;}setVendors(vv=>[...vv,Array.isArray(r)?r[0]:r[0]||r]);}}}catch(e){alert("오류 발생");return;}setSheet(false);}
...
  async function saveProfile(){try{if(user?.token)await DB.updateUser(user.token,{name:pf.name,company:pf.company,tel:pf.tel});}catch{}if(setUser)setUser(u=>({...u,...pf}));setProfileSheet(false);alert("저장되었습니다!");}
  // [수리 6] 가짜 데이터 주입(uid) 삭제, 오류 시 에러창 표시
  async function saveFac(){if(!facSheet.name)return;const{id,...data}=facSheet;try{if(id){if(user?.token){const r=await DB.update(user.token,"factories",id,{...data,biz_type:data.bizType});if(r.error||r.message){alert("오류: "+r.message);return;}setFactories(ff=>ff.map(x=>x.id===id?{...x,...data}:x));}}else{if(user?.token){const r=await DB.insert(user.token,"factories",{name:data.name,biz_type:data.bizType,address:data.address,tel:data.tel,account:data.account,user_id:user.id});if(r.error||r.message){alert("오류: "+r.message);return;}setFactories(ff=>[...ff,Array.isArray(r)?{...r[0],bizType:r[0].biz_type||""}:r]);}}}catch(e){alert("오류 발생");return;}setFacSheet(null);}
...
  const [loading,setLoading]=useState(false);
  ...
  // [수리 7] 내 데이터만 가져오도록 uid 전달
  async function loadData(token, uid){
    setLoading(true);
    try{
      const[v,f,p,o]=await Promise.all([DB.list(token,"vendors",uid),DB.list(token,"factories",uid),DB.list(token,"products",uid),DB.list(token,"orders",uid)]);
...
  }
  ...
  // [수리 8] 새로고침 시 확실하게 데이터 로딩
  useEffect(()=>{
    try{const s=localStorage.getItem("dworks_session");if(s){const u=JSON.parse(s);if(u?.token){setUser(u);setScreen("app");loadData(u.token, u.id);return;}}}catch{}
    setScreen("splash");
  },[]);

  async function handleLogin(u){try{localStorage.setItem("dworks_session",JSON.stringify(u));}catch{}setUser(u);setScreen("app");await loadData(u.token, u.id);}
