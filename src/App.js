...
const C={bg:"#F4F7FB",bg2:"#EAF1FF",card:"#FFFFFF",bdr:"#E4EAF3",line:"#EDF2F8",acc:"#2563EB",acc2:"#0EA5E9",txt:"#0F172A",sub:"#94A3B8",sub2:"#475569",ok:"#10B981",warn:"#F59E0B",red:"#EF4444",fn:"'Noto Sans KR','Apple SD Gothic Neo',sans-serif"};
...
const btnTone=(v,disabled)=>{
  ...
  return {bg:`linear-gradient(135deg, ${C.acc} 0%, ${C.acc2} 100%)`,cl:"#fff",bd:"none",sh:"0 14px 32px rgba(37,99,235,0.24)"};
};
  ...
  return(
    <div style={{padding:"18px 14px 104px"}}>
      <div style={{marginBottom:16}}>
        <div style={{fontSize:12,fontWeight:800,color:C.sub2,marginBottom:6}}>Dashboard</div>
        <div style={{fontWeight:900,fontSize:24,color:C.txt,letterSpacing:"-0.03em"}}>대시보드</div>
      </div>

      <div style={{borderRadius:30,background:`linear-gradient(135deg, ${C.acc} 0%, ${C.acc2} 100%)`,padding:"20px 18px",color:"#fff",boxShadow:"0 24px 60px rgba(37,99,235,0.18)",marginBottom:14}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10}}>
          <div>
            <div style={{fontSize:12,fontWeight:800,opacity:.78,marginBottom:8}}>Order overview</div>
            <div style={{fontSize:24,fontWeight:900,letterSpacing:"-0.03em",lineHeight:1.28}}>익숙한 구조는 유지하고<br/>화면 밀도만 바꿉니다</div>
          </div>
          <div style={{background:"rgba(255,255,255,0.14)",border:"1px solid rgba(255,255,255,0.18)",padding:"10px 12px",borderRadius:18,textAlign:"right",backdropFilter:"blur(10px)"}}>
            <div style={{fontSize:11,opacity:.76}}>오늘 기준</div>
            <div style={{fontSize:16,fontWeight:900,marginTop:3}}>{td.slice(5).replace("-","/")}</div>
          </div>
        </div>
        <div style={{display:"flex",gap:8,marginTop:16,flexWrap:"wrap"}}>
          <span style={{background:"rgba(255,255,255,0.12)",border:"1px solid rgba(255,255,255,0.15)",padding:"6px 10px",borderRadius:999,fontSize:11,fontWeight:800}}>오늘 발주 {tO.length}건</span>
          <span style={{background:"rgba(255,255,255,0.12)",border:"1px solid rgba(255,255,255,0.15)",padding:"6px 10px",borderRadius:999,fontSize:11,fontWeight:800}}>미출고 {delayed.length}건</span>
          <span style={{background:"rgba(255,255,255,0.12)",border:"1px solid rgba(255,255,255,0.15)",padding:"6px 10px",borderRadius:999,fontSize:11,fontWeight:800}}>월간 {fmtN(mQ)}매</span>
        </div>
      </div>

      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
        <Card st={{padding:16,marginBottom:0}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            <span style={{fontSize:12,fontWeight:800,color:C.sub2}}>오늘 발주</span>
            <div style={{width:38,height:38,borderRadius:14,background:C.bg2,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>📦</div>
          </div>
          <div style={{fontSize:28,fontWeight:900,color:C.txt,letterSpacing:"-0.03em"}}>{tO.length}건</div>
          <div style={{marginTop:6,fontSize:12,fontWeight:700,color:diff>=0?C.ok:C.warn}}>{diff>=0?`전일 대비 +${fmtN(diff)}매`:`전일 대비 ${fmtN(diff)}매`}</div>
        </Card>
        <Card st={{padding:16,marginBottom:0}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            <span style={{fontSize:12,fontWeight:800,color:C.sub2}}>미출고 발주</span>
            <div style={{width:38,height:38,borderRadius:14,background:"#FFF7ED",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>⏳</div>
          </div>
          <div style={{fontSize:28,fontWeight:900,color:C.txt,letterSpacing:"-0.03em"}}>{delayed.length}건</div>
          <div style={{marginTop:6,fontSize:12,fontWeight:700,color:delayed.length?C.warn:C.ok}}>{delayed.length?"빠른 확인 필요":"안정적으로 관리 중"}</div>
        </Card>
      </div>
      ...
      <Card st={{marginBottom:12}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          <div>
            ...
            <div style={{fontSize:30,fontWeight:900,color:C.txt,letterSpacing:"-0.03em"}}>{fmtN(mQ)}매</div>
          </div>
          <div style={{background:"#ECFDF5",color:C.ok,padding:"6px 10px",borderRadius:999,fontSize:11,fontWeight:800,border:`1px solid ${C.ok}22`}}>최근 7일 기준</div>
        </div>
        <LineChart data={chart}/>
        <div style={{display:"flex",justifyContent:"space-between",marginTop:4}}>{chart.map((d,i)=><span key={d.label} style={{fontSize:10,color:i%2===0?C.sub2:C.sub}}>{i%2===0?d.label:""}</span>)}</div>
      </Card>

      <Card>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          <span style={{fontWeight:900,fontSize:15,color:C.txt}}>지연 발주</span>
          {delayed.length>2?<button onClick={()=>onNav("list")} style={{background:"none",border:"none",fontSize:12,color:C.sub2,cursor:"pointer",fontFamily:C.fn,fontWeight:700}}>전체보기</button>:<Tag ch={`총 ${delayed.length}건`} c={delayed.length?C.warn:C.ok}/>}          
        </div>
        {delayed.length===0?<div style={{textAlign:"center",padding:"18px 0",color:C.sub2,fontSize:13,fontWeight:700}}>지연 발주가 없습니다 ✅</div>:<div style={{display:"flex",flexDirection:"column",gap:8}}>{delayed.slice(0,5).flatMap(o=>(o.items||[]).map((it,j)=>{const p=products.find(x=>x.id===it.pid);return(
          <div key={`${o.id}-${j}`} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 14px",border:`1px solid ${C.bdr}`,borderRadius:18,background:"#FCFCFD"}}>
      ...
      <div style={{borderRadius:30,background:`linear-gradient(135deg, #0F172A 0%, #1E293B 70%, ${C.acc} 100%)`,padding:"20px 18px",color:"#fff",boxShadow:"0 24px 60px rgba(15,23,42,0.20)",marginBottom:14}}>
        <div style={{display:"flex",alignItems:"center",gap:14}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:12,fontWeight:800,opacity:.72,marginBottom:4}}>Profile</div>
...
export default function App(){
  ...
  const TABS=[{k:"order", l:"발주하기", ic:"📝"},{k:"prods", l:"상품", ic:"👕"},{k:"list", l:"발주리스트", ic:"📋"},{k:"vendors", l:"거래처", ic:"🏭"},{k:"settings", l:"설정", ic:"⚙️"}];
  ...
  return(
    <div style={{minHeight:"100vh",background:`linear-gradient(180deg, #EFF4FF 0%, #F8FAFC 100%)`,fontFamily:C.fn,padding:"0",position:"relative"}}>
      <div style={{maxWidth:430,margin:"0 auto",minHeight:"100vh",position:"relative",background:`linear-gradient(180deg, #FFFFFF 0%, ${C.bg} 100%)`,boxShadow:"0 28px 90px rgba(15,23,42,0.14)",overflow:"hidden"}}>
        <div style={{background:"rgba(255,255,255,0.82)",backdropFilter:"blur(14px)",padding:"14px 18px",display:"flex",justifyContent:"space-between",alignItems:"center",position:"sticky",top:0,zIndex:50,borderBottom:`1px solid ${C.bdr}`}}>
          <button onClick={()=>setPage("dash")} style={{background:"none",border:"none",color:C.acc,fontWeight:900,fontSize:22,cursor:"pointer",fontFamily:C.fn,letterSpacing:0.4,padding:0}}>D-Works</button>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <span style={{padding:"7px 10px",borderRadius:999,background:C.bg,border:`1px solid ${C.bdr}`,color:C.sub2,fontSize:11,fontWeight:800,maxWidth:120,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{user.brand ? user.brand : user.name}</span>
          </div>
        </div>
        <div style={{paddingBottom:112}}>{pages[page]||pages["dash"]}</div>
        
        <div style={{position:"fixed",bottom:14,left:"50%",transform:"translateX(-50%)",width:"calc(100% - 24px)",maxWidth:406,zIndex:60,pointerEvents:"none"}}>
          <div style={{background:"rgba(255,255,255,0.88)",backdropFilter:"blur(18px)",border:`1px solid ${C.bdr}`,borderRadius:28,padding:8,display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:6,boxShadow:"0 18px 40px rgba(15,23,42,0.14)",pointerEvents:"auto"}}>
            {TABS.map(t=>{
              const active=page===t.k;
              return <button key={t.k} onClick={()=>setPage(t.k)} style={{background:active?"#fff":"transparent",border:active?`1px solid ${C.bdr}`:"1px solid transparent",borderRadius:20,color:active?C.txt:C.sub2,cursor:"pointer",fontFamily:C.fn,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"10px 4px",transition:"all 0.2s",boxShadow:active?"0 10px 24px rgba(15,23,42,0.08)":"none"}}><span style={{fontSize:15,marginBottom:4}}>{t.ic}</span><span style={{fontSize:10.5,fontWeight:active?900:700,lineHeight:1.15,textAlign:"center"}}>{t.l}</span></button>;
            })}
          </div>
        </div>
      </div>
