...
// [수정1] API 에러 발생 시 가짜로 저장된 척하지 않고 명확히 에러를 던지도록 수정
async function api(m,p,t,b){const r=await fetch(`${SB}${p}`,{method:m,headers:ah(t),body:b?JSON.stringify(b):undefined});const j=await r.json();if(!r.ok)throw new Error(j.message||"API 오류");return j;}
const DB={
...
  updateUser:(t,meta)=>api("PUT","/auth/v1/user",t,{data:meta}),
  // [수정2] 내 데이터만 확실히 가져오도록 user_id 필터 추가
...
  function addItem(){if(!selProd||!selColor||!qty){alert("상품·색상·수량을 입력하세요");return;}const idx=items.findIndex(i=>i.pid===selProd.id&&i.color===selColor);if(idx>=0)setItems(p=>p.map((it,i)=>i===idx?{...it,qty:it.qty+Number(qty)}:it));else setItems(p=>[...p,{pid:selProd.id,color:selColor,qty:Number(qty)}]);setSelProd(null);setSelColor("");setQty("");setSearch("");}
  // [수정3] 발주 저장 시 실패하면 가짜 데이터 넣지 않음
  async function submit(){if(!items.length){alert("발주 항목 추가");return;}const o={items,status:"진행중",date:today(),ts:new Date().toISOString()};try{if(user?.token){const r=await DB.insert(user.token,"orders",{...o,user_id:user.id});setOrders(p=>[...p,r[0]]);}try{localStorage.removeItem(DRAFT);}catch{}setStep(3);}catch(e){alert("저장 실패: "+e.message);}}
...
  function addBom(){if(!br.mat||!br.amt)return;if(editBomId){setF(p=>({...p,bom:p.bom.map(b=>b.id===editBomId?{...b,...br,amt:Number(br.amt)}:b)}));setEditBomId(null);}else setF(p=>({...p,bom:[...p.bom,{...br,id:uid(),amt:Number(br.amt)}]}));setBr({type:"",mat:"",amt:"",vid:""});setVenSearch("");}
  // [수정4] 상품 저장 시 실패하면 가짜 데이터 넣지 않음
  async function save(){
    if(!f.name)return;
    try{if(f.id&&user?.token){await DB.update(user.token,"products",f.id,{name:f.name,category:f.category,season:f.season,factory_id:f.factoryId,factory:f.factory,factory_tel:f.factoryTel,colors:f.colors,bom:f.bom});setProducts(products.map(p=>p.id===f.id?f:p));}else if(user?.token){const r=await DB.insert(user.token,"products",{name:f.name,category:f.category,season:f.season,factory_id:f.factoryId,factory:f.factory,factory_tel:f.factoryTel,colors:f.colors,bom:f.bom,user_id:user.id});setProducts(p=>[...p,{...r[0],factoryId:r[0].factory_id||"",factoryTel:r[0].factory_tel||"",colors:r[0].colors||[],bom:r[0].bom||[]}]);}setSheet(false);}catch(e){alert("저장 실패: "+e.message);}
  }
...
  function openEdit(v){setF({...v});setEditId(v.id);setSheet(true);}
  // [수정5] 거래처 저장 시 실패하면 가짜 데이터 넣지 않음
  async function save(){if(!f.name)return;try{if(editId){if(user?.token){await DB.update(user.token,"vendors",editId,{name:f.name,tel:f.tel,email:f.email,type:f.type});setVendors(vv=>vv.map(v=>v.id===editId?{...v,...f}:v));}}else{if(user?.token){const r=await DB.insert(user.token,"vendors",{name:f.name,tel:f.tel,email:f.email,type:f.type,user_id:user.id});setVendors(vv=>[...vv,r[0]]);}}}catch(e){alert("저장 실패: "+e.message);return;}setSheet(false);}
...
  async function saveProfile(){try{if(user?.token)await DB.updateUser(user.token,{name:pf.name,company:pf.company,tel:pf.tel});}catch{}if(setUser)setUser(u=>({...u,...pf}));setProfileSheet(false);alert("저장되었습니다!");}
  // [수정6] 공장 저장 시 실패하면 가짜 데이터 넣지 않음
  async function saveFac(){if(!facSheet.name)return;const{id,...data}=facSheet;try{if(id){if(user?.token){await DB.update(user.token,"factories",id,{...data,biz_type:data.bizType});setFactories(ff=>ff.map(x=>x.id===id?{...x,...data}:x));}}else{if(user?.token){const r=await DB.insert(user.token,"factories",{name:data.name,biz_type:data.bizType,address:data.address,tel:data.tel,account:data.account,user_id:user.id});setFactories(ff=>[...ff,{...r[0],bizType:r[0].biz_type||""}]);}}setFacSheet(null);}catch(e){alert("저장 실패: "+e.message);}}
...
  const [loading,setLoading]=useState(false);
  ...
  // [수정7] 데이터를 가져올 때 로그인된 사용자의 ID를 사용해 필터링
...
  }
  ...
  // [수정8] 세션 읽은 직후 loadData 실행하여 새로고침 시 데이터 불러오기 보장
  useEffect(()=>{
