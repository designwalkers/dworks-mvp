...
const KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InFpbWdvc3Rpc2VlaGRudmhtb3BoIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzUwMTQ1NDgsImV4cCI6MjA5MDU5MDU0OH0.7upLxWR1OqwvIx71Z4pFHUU7BFswDvcOQE9edjcL2yg";
...
const DB = {
  ...
  signIn: (email, password) =>
    api("POST", "/auth/v1/token?grant_type=password", null, {
      email,
      password,
    }),

  signOut: (token) => api("POST", "/auth/v1/logout", token),

  updateUser: (token, meta) =>
    api("PUT", "/auth/v1/user", token, {
      data: meta,
    }),

  list: (token, table, userId) =>
    api(
      "GET",
      `/rest/v1/${table}?select=*&user_id=eq.${encodeURIComponent(userId)}`,
      token
    ),

  insert: (token, table, data) => api("POST", `/rest/v1/${table}`, token, data),

  update: (token, table, id, userId, data) =>
    api(
      "PATCH",
      `/rest/v1/${table}?id=eq.${encodeURIComponent(
        id
...
const EJS = {
  SID: "service_raca1ke",
  TID: "template_hoej0ts",
  PK: "KlYRj7B6JNO01D2pm",
};
  ...
  return (
    <button
      ...
      style={{
        ...
        color: disabled ? "#B0B8C4" : cl,
        border: disabled ? `1.5px solid ${C.bdr}` : bd,
        borderRadius: 10,
        padding: sz === "s" ? "7px 14px" : "12px 0",
        fontSize: sz === "s" ? 12 : 14,
        fontWeight: 700,
...
function FRow({ label, children, last, req }) {
  return (
    ...
    >
      ...
      >
        {label}
        {req && <span style={{ color: C.acc, marginLeft: 2 }}>*</span>}
      </div>
      <div
        style={{
          flex: 1,
...
const FInp = ({ val, onChange, ph, type = "text" }) => (
  <input
    value={val || ""}
...
const Card = ({ children, st = {}, onClick }) => (
  <div
    onClick={onClick}
    style={{
      background: "#fff",
...
const Empty = ({ icon, text }) => (
  <div style={{ textAlign: "center", padding: "40px 20px", color: C.sub }}>
    ...
    <div style={{ fontSize: 14, fontWeight: 600, color: C.sub2 }}>{text}</div>
  </div>
);
...
function LineChart({ data }) {
  ...
  const pts = data.map((d, i) => [
    ...
    H - p - ((d.v - mn) / rng) * (H - p * 2),
  ]);

  const area = `M ${pts[0][0]},${H - p} ${pts
    .map(([x, y]) => `L ${x},${y}`)
    .join(" ")} L ${pts[pts.length - 1][0]},${H - p} Z`;
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: H }}>
      ...
      />
      {pts.map(([x, y], i) => (
        <circle key={i} cx={x} cy={y} r="3" fill={C.acc} />
      ))}
    </svg>
  );
}
...
function SplashPage({ onStart }) {
  ...
  const slides = [
    ...
    {
      title: "거래처별\n원클릭 발송",
      desc: "업체별 발주서를 자동 생성하고\n이메일로 즉시 발송합니다.",
      icon: "📧",
    },
    {
      title: "발주 이력\n데이터화",
      desc: "모든 발주가 DB에 저장되어\n언제든 조회 가능합니다.",
      icon: "📊",
    },
  ];
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ color: C.acc, fontWeight: 900, fontSize: 24, letterSpacing: 1 }}>
          D-Works
        </div>
        <button
          onClick={onStart}
          ...
          {slides.map((_, i) => (
            <div
              ...
              style={{
                ...
                height: 8,
                borderRadius: 4,
                background: i === slide ? C.acc : C.bdr,
                cursor: "pointer",
                transition: "all 0.3s",
              }}
            />
...
function AuthPage({ onLogin }) {
  ...
  async function submit() {
    ...
    if (tab === "up" && f.pw !== f.pw2) {
      setErr("비밀번호가 일치하지 않습니다");
      return;
    }

    if (tab === "up" && f.pw.length < 6) {
      setErr("비밀번호는 6자 이상 입력해주세요");
      return;
    }
    ...
    try {
      if (tab === "up") {
        ...
        if (isErr(r)) {
          ...
          setErr(msg.includes("already") ? "이미 가입된 이메일입니다" : msg);
          return;
        }

        if (r?.user && !r?.session) {
          setErr("회원가입이 완료되었습니다. 이메일 인증 후 로그인해주세요.");
          setTab("in");
          return;
        }
        ...
        if (isErr(r2) || !r2?.access_token) {
          const msg = r2?.error?.message || "";
          if (/email not confirmed/i.test(msg)) {
            setErr("이메일 인증 후 로그인해주세요.");
          } else {
            setErr(msg || "로그인 실패");
          }
          return;
        ...
        if (isErr(r) || !r?.access_token) {
          const msg = r?.error?.message || "";
          if (/email not confirmed/i.test(msg)) {
            setErr("이메일 인증 후 로그인해주세요.");
          } else if (/invalid/i.test(msg)) {
            setErr("이메일 또는 비밀번호가 틀렸습니다");
          } else {
            setErr(msg || "로그인 실패");
          }
          return;
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: C.fn }}>
      ...
      >
        <div style={{ fontSize: 30, fontWeight: 900, color: C.acc, letterSpacing: 1 }}>
          D-Works
        </div>
      ...
      <div style={{ padding: "20px 20px 40px", maxWidth: 480, margin: "0 auto" }}>
        ...
        {tab === "up" && (
          <>
            <Field label="이름" req>
              <TxtInp val={f.name} onChange={sf("name")} ph="이름 입력" />
            </Field>
            <Field label="업체명">
              <TxtInp val={f.company} onChange={sf("company")} ph="업체명 입력" />
            </Field>
          </>
        )}

        <Field label="이메일" req>
          <TxtInp val={f.email} onChange={sf("email")} ph="이메일" type="email" />
        </Field>

        <Field label="비밀번호" req>
          <TxtInp
            val={f.pw}
            onChange={sf("pw")}
            ph={tab === "up" ? "6자 이상" : "비밀번호"}
            type="password"
        ...
        {tab === "up" && (
          <>
            ...
            <Field label="연락처">
              <TxtInp val={f.tel} onChange={sf("tel")} ph="010-0000-0000" type="tel" />
            </Field>
          </>
        )}
        ...
        <Btn
          ch={loading ? (tab === "in" ? "로그인 중..." : "가입 중...") : tab === "in" ? "로그인" : "가입하기"}
          onClick={submit}
          full
          sz="l"
          disabled={loading}
          st={{ borderRadius: 10, height: 50, fontSize: 15 }}
        />
        ...
        {tab === "in" && (
          <div style={{ textAlign: "center", marginTop: 14, fontSize: 13, color: C.sub }}>
            ...
            <span
              onClick={() => setTab("up")}
              style={{ color: C.acc, fontWeight: 700, cursor: "pointer" }}
            >
              회원가입
            </span>
          </div>
...
function DashPage({ orders, products, onNav }) {
  ...
  const mQ = orders
    .filter((o) => o.date?.slice(0, 7) === td.slice(0, 7))
    .reduce((s, o) => s + (o.items || []).reduce((ss, i) => ss + (i.qty || 0), 0), 0);
  ...
  const chart = Array.from({ length: 7 }, (_, i) => {
    ...
    return {
      ...
      v: orders
        .filter((o) => o.date === ds)
        .reduce((s, o) => s + (o.items || []).reduce((ss, ii) => ss + (ii.qty || 0), 0), 0),
    };
  });

  return (
    <div style={{ padding: "14px 14px 80px" }}>
      <div style={{ fontWeight: 900, fontSize: 20, textAlign: "center", marginBottom: 14 }}>
        대시 보드
      </div>
      ...
      >
        {[
          ...
          { label: "미출고 발주", val: `${delayed.length}건`, c: "#8B5CF6" },
          ...
          >
            ...
            <div style={{ color: C.sub, fontSize: 10, marginTop: 3, fontWeight: 600 }}>
              {s.label}
            </div>
          </div>
        ))}
      ...
      <Card>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
          {chart
            .filter((_, i) => i % 2 === 0)
            .map((d) => (
              <span key={d.label} style={{ fontSize: 9, color: C.sub }}>
                {d.label}
              </span>
            ))}
        </div>
...
function OrderPage({ products, setOrders, vendors, user }) {
  ...
  const filtered = products.filter(
    (p) => p.name?.includes(search) || p.season?.includes(search)
  );
  ...
  function addItem() {
    ...
    if (idx >= 0) {
      setItems((p) =>
        p.map((it, i) => (i === idx ? { ...it, qty: it.qty + nQty } : it))
      );
    } else {
      setItems((p) => [...p, { pid: selProd.id, color: selColor, qty: nQty }]);
    }
  ...
  async function submit() {
    ...
    try {
      const r = await DB.insert(user.token, "orders", o);

      if (isErr(r)) {
        alert(r.error.message || "발주 저장 실패");
        return;
      }

      setOrders((p) => [...p, Array.isArray(r) ? r[0] : { ...o, id: uid() }]);
    } catch {
      setOrders((p) => [...p, { ...o, id: uid() }]);
    }
  ...
  async function sendMail() {
    ...
    for (const it of items) {
      ...
      for (const b of prod.bom || []) {
        ...
        const soyo = Math.round((b.amt || 0) * it.qty * 100) / 100;

        if (!venMap[ven.id]) venMap[ven.id] = { vendor: ven, entries: [] };

        venMap[ven.id].entries.push({
          prod,
          b,
          color: it.color,
          soyo,
    ...
    if (cnt > 0) {
      alert(`✅ ${cnt}곳 거래처에 발주서를 발송했습니다!`);
    } else {
      alert("발송 실패. 거래처 이메일을 확인해주세요.");
    }
  }
  ...
  if (step === 3) {
    return (
      ...
      >
        ...
        <div style={{ color: C.sub, marginBottom: 28, fontSize: 13 }}>
          {items.length}개 상품 발주
        </div>
      ...
      {step === 1 && (
        <>
            ...
            <Field label="상품명">
              <div style={{ position: "relative" }}>
                <TxtInp
                  ...
                  onChange={(v) => {
                    setSearch(v);
                    if (selProd && v !== selProd.name) setSelProd(null);
                  }}
                />
          ...
          <Card st={{ marginBottom: 18 }}>
            {items.length === 0 ? (
              <div style={{ padding: "16px 0", color: C.sub, fontSize: 12, textAlign: "center" }}>
                추가된 항목 없음
              </div>
            ) : (
              items.map((it, i) => {
                ...
                return (
                  ...
                  >
                    <div style={{ fontSize: 13 }}>
                      <span style={{ fontWeight: 700 }}>{p?.name}</span> / {it.color}
                    </div>
                    <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                      <span style={{ fontWeight: 700, color: C.acc, fontSize: 13 }}>
                        {fmtN(it.qty)}장
                      </span>
                      <button
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <Btn
              ...
              full
              st={{ flex: 2 }}
              onClick={() => (items.length ? setStep(2) : alert("항목 추가 필요"))}
        ...
        <>
          <Card st={{ marginBottom: 12 }}>
            ...
            {items.map((it, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span style={{ fontWeight: 800, color: C.acc, fontSize: 15 }}>
                    {fmtN(it.qty)}장
                  </span>
                </div>
              );
            ...
            <div style={{ display: "flex", justifyContent: "space-between", paddingTop: 10 }}>
              ...
              <span style={{ fontWeight: 900, color: C.acc, fontSize: 17 }}>
                {fmtN(items.reduce((s, it) => s + it.qty, 0))}장
              </span>
            </div>
          </Card>

          <Btn
            ch={sending ? "발송 중..." : "📧 이메일 발송"}
            v="w"
            full
            st={{ marginBottom: 10 }}
            onClick={sendMail}
...
function ProdsPage({ products, setProducts, vendors, factories, user }) {
  ...
  function addBom() {
    ...
    const nextBom = {
      ...
      id: editBomId || uid(),
      amt: Number(br.amt),
      price: br.price ? Number(br.price) : 0,
    };

    if (editBomId) {
      setF((p) => ({
        ...p,
        bom: p.bom.map((b) => (b.id === editBomId ? nextBom : b)),
      }));
      setEditBomId(null);
    } else {
      setF((p) => ({
  ...
  async function save() {
    ...
    try {
      if (f.id) {
        const r = await DB.update(user.token, "products", f.id, user.id, payload);
        if (isErr(r)) {
          alert(r.error.message || "상품 수정 실패");
          return;
        }

        setProducts((prev) => prev.map((p) => (p.id === f.id ? { ...f } : p)));
      } else {
        const r = await DB.insert(user.token, "products", payload);
        if (isErr(r)) {
          alert(r.error.message || "상품 저장 실패");
          return;
        }

        const saved = Array.isArray(r)
          ? {
              ...r[0],
              factoryId: r[0].factory_id || "",
              factoryTel: r[0].factory_tel || "",
              colors: r[0].colors || [],
              bom: r[0].bom || [],
            }
          : { ...f, id: uid() };
  ...
  async function del(id) {
    ...
    const r = await DB.del(user.token, "products", id, user.id);
    if (isErr(r) && r.status !== 200 && r.status !== 204) {
      alert(r.error.message || "삭제 실패");
      return;
    }
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ color: C.sub, fontSize: 11 }}>
                  {p.factory && `📍 ${p.factory}`}
                  {p.bom?.length > 0 && ` · BOM ${p.bom.length}종`}
                </div>
              </div>
              ...
              <div style={{ display: "flex", gap: 6, flexShrink: 0, marginLeft: 8 }}>
                <Btn
                  ...
                  v="w"
                  sz="s"
                  st={{ padding: "5px 11px", fontSize: 12 }}
                  onClick={() => openEdit(p)}
                />
                <Btn
                  ch="삭제"
                  v="w"
                  sz="s"
                  st={{ padding: "5px 11px", fontSize: 12, color: C.red }}
                  onClick={() => del(p.id)}
                />
              </div>
            </div>
      ...
      {sheet && (
        <Sheet title={f.id ? "상품 수정" : "1단계 원단 입력"} onClose={() => setSheet(false)}>
          ...
          <FCard>
            ...
            <FRow label="시즌">
              <FSel val={f.season} onChange={sf("season")} ph="">
                {SEASONS.map((s) => (
          ...
          {f.factory && (
            <div style={{ fontSize: 11, color: C.sub, marginBottom: 10, marginTop: -6 }}>
              📞 {f.factoryTel} · 발주서 자동포함
            </div>
          )}
          ...
          <FCard>
            ...
            <FRow label="소요량">
              <FInp
                val={br.amt}
                onChange={(v) => setBr((r) => ({ ...r, amt: v }))}
                ph="0.0"
                type="number"
              />
              <span style={{ color: C.sub, fontSize: 11, flexShrink: 0, marginLeft: 4 }}>yd</span>
            </FRow>

            <FRow label="단가" last>
              <FInp
                val={br.price || ""}
                onChange={(v) => setBr((r) => ({ ...r, price: v }))}
                ph="0"
                type="number"
              />
              <span style={{ color: C.sub, fontSize: 11, flexShrink: 0, marginLeft: 4 }}>원</span>
            </FRow>
          </FCard>
            ...
            f.bom.map((b) => {
              ...
              return (
                <div
                  key={b.id}
                  style={{
                    background: isE ? C.acc + "10" : C.bg,
                    borderRadius: 8,
                    padding: "9px 12px",
                    marginBottom: 6,
                    border: `1.5px solid ${isE ? C.acc : C.bdr}`,
                  }}
                >
...
function ListPage({ orders, setOrders, products, user }) {
  ...
  const filtered = (filter === "전체" ? orders : orders.filter((o) => o.status === filter)).sort(
    (a, b) => new Date(b.ts || 0) - new Date(a.ts || 0)
  );

  async function changeStatus(id, status) {
    const r = await DB.update(user.token, "orders", id, user.id, { status });
    if (isErr(r)) {
      alert(r.error.message || "상태 변경 실패");
      return;
    }
  ...
  async function delOrder(id) {
    if (!window.confirm("삭제할까요?")) return;
    const r = await DB.del(user.token, "orders", id, user.id);
    if (isErr(r) && r.status !== 200 && r.status !== 204) {
      alert(r.error.message || "삭제 실패");
      return;
    }
                  ...
                  {(o.items || []).map((it, j) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span>
                          {it.color} · <strong>{fmtN(it.qty)}</strong>장
                        </span>
                      </div>
                    );
                  })}
                  ...
                  <div style={{ display: "flex", gap: 7, marginTop: 10, flexWrap: "wrap" }}>
                    {["진행중", "완료", "지연"].map((st) => (
                      <Btn
                        ...
                        st={{
                          padding: "5px 11px",
                          background: o.status === st ? SC[st] || C.acc : "#fff",
                          color: o.status === st ? "#fff" : SC[st] || C.sub2,
                    ...
                    <Btn
                      ...
                      sz="s"
                      v="w"
                      st={{ marginLeft: "auto", color: C.red, fontSize: 12, padding: "5px 11px" }}
...
function VendorPage({ vendors, setVendors, user }) {
  ...
  async function save() {
    ...
    if (editId) {
      const r = await DB.update(user.token, "vendors", editId, user.id, payload);
      if (isErr(r)) {
        alert(r.error.message || "거래처 수정 실패");
        return;
      }
      setVendors((vv) => vv.map((v) => (v.id === editId ? { ...v, ...f } : v)));
    } else {
      const r = await DB.insert(user.token, "vendors", payload);
      if (isErr(r)) {
        alert(r.error.message || "거래처 저장 실패");
        return;
      }
      setVendors((vv) => [...vv, Array.isArray(r) ? r[0] : { ...f, id: uid() }]);
    }
  ...
  async function del(id) {
    if (!window.confirm("삭제할까요?")) return;
    const r = await DB.del(user.token, "vendors", id, user.id);
    if (isErr(r) && r.status !== 200 && r.status !== 204) {
      alert(r.error.message || "삭제 실패");
      return;
    }
    ...
    <div style={{ padding: "14px 14px 80px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <div
                style={{
                  ...
                  height: 40,
                  borderRadius: 12,
                  background: (VEN_C[v.type] || C.sub) + "18",
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
                  fontSize: 18,
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 11, color: v.email ? C.sub : C.warn, marginTop: 2 }}>
                  {v.email || "⚠️ 이메일 미등록"}
                </div>
              </div>
      ...
      {sheet && (
        <Sheet title={editId ? "거래처 수정" : "거래처 추가"} onClose={() => setSheet(false)}>
          <Field label="거래처명" req>
            <TxtInp val={f.name} onChange={sf("name")} ph="이레텍스" />
          </Field>
          <Field label="전화번호">
            <TxtInp val={f.tel} onChange={sf("tel")} ph="010-0000-0000" type="tel" />
          </Field>
          <Field label="이메일 (발주서 발송용)">
            <TxtInp val={f.email} onChange={sf("email")} ph="order@fabric.com" type="email" />
          </Field>
          <Field label="업체 유형">
...
function SettingsPage({ user, setUser, factories, setFactories, onLogout }) {
  ...
  async function saveProfile() {
    const r = await DB.updateUser(user.token, {
      name: pf.name,
      company: pf.company,
      tel: pf.tel,
    });
    ...
    if (isErr(r)) {
      alert(r.error.message || "프로필 저장 실패");
      return;
    }
    ...
    if (setUser) {
      ...
      try {
        const next = { ...user, ...pf };
        localStorage.setItem("dworks_session", JSON.stringify(next));
      } catch {}
    }
  ...
  async function saveFac() {
    ...
    if (id) {
      const r = await DB.update(user.token, "factories", id, user.id, payload);
      if (isErr(r)) {
        alert(r.error.message || "공장 수정 실패");
        return;
      }
      setFactories((ff) => ff.map((x) => (x.id === id ? { ...x, ...data } : x)));
    } else {
      const r = await DB.insert(user.token, "factories", payload);
      if (isErr(r)) {
        alert(r.error.message || "공장 저장 실패");
        return;
      }
      setFactories((ff) => [
        ...ff,
        Array.isArray(r)
          ? { ...r[0], bizType: r[0].biz_type || "" }
          : { ...data, id: uid() },
      ]);
    }
  ...
  async function delFac(id) {
    if (!window.confirm("삭제할까요?")) return;
    const r = await DB.del(user.token, "factories", id, user.id);
    if (isErr(r) && r.status !== 200 && r.status !== 204) {
      alert(r.error.message || "삭제 실패");
      return;
    }
    ...
    <div style={{ padding: "14px 14px 80px" }}>
      ...
      <Card>
        ...
        >
          ...
          <Btn
            ...
            st={{ padding: "5px 11px", fontSize: 12 }}
            onClick={() =>
              setFacSheet({ id: null, name: "", bizType: "", address: "", tel: "", account: "" })
            }
          />
        </div>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                ...
                <div style={{ display: "flex", gap: 5, flexShrink: 0, marginLeft: 8 }}>
                  <Btn
                    ...
                    v="w"
                    sz="s"
                    st={{ padding: "4px 10px", fontSize: 11 }}
                    onClick={() => setFacSheet({ ...fc, bizType: fc.bizType || fc.biz_type || "" })}
                  />
                  <Btn
                    ch="삭제"
                    v="w"
                    sz="s"
                    st={{ padding: "4px 10px", fontSize: 11, color: C.red }}
                    onClick={() => delFac(fc.id)}
                  />
                </div>
              </div>
            </div>
      ...
      {facSheet !== null && (
        <Sheet title={facSheet.id ? "공장 수정" : "공장 추가"} onClose={() => setFacSheet(null)}>
          <Field label="공장명" req>
            <TxtInp
              val={facSheet.name || ""}
              onChange={(v) => setFacSheet((p) => ({ ...p, name: v }))}
              ph="예: OO봉제"
            />
          </Field>
          <Field label="업종">
            <DropSel
              val={facSheet.bizType || ""}
              onChange={(v) => setFacSheet((p) => ({ ...p, bizType: v }))}
              ph="업종 선택"
            >
              {BIZ_TYPES.map((t) => (
          ...
          <Field label="주소">
            <TxtInp
              val={facSheet.address || ""}
              onChange={(v) => setFacSheet((p) => ({ ...p, address: v }))}
              ph="서울시 중구 OO동"
            />
          </Field>
          <Field label="연락처">
            <TxtInp
              val={facSheet.tel || ""}
              onChange={(v) => setFacSheet((p) => ({ ...p, tel: v }))}
              ph="02-0000-0000"
              type="tel"
            />
          </Field>
          <Field label="계좌번호">
            <TxtInp
              val={facSheet.account || ""}
              onChange={(v) => setFacSheet((p) => ({ ...p, account: v }))}
              ph="은행명 계좌번호 예금주"
            />
          </Field>
      ...
      {profileSheet && (
        <Sheet title="프로필 수정" onClose={() => setProfileSheet(false)}>
          <Field label="이름" req>
            <TxtInp val={pf.name} onChange={(v) => setPf((p) => ({ ...p, name: v }))} ph="이름" />
          </Field>
          <Field label="업체명">
            <TxtInp
              val={pf.company}
              onChange={(v) => setPf((p) => ({ ...p, company: v }))}
              ph="업체명"
            />
          </Field>
          <Field label="연락처">
...
export default function App() {
  ...
  function clearSessionAndReset(to = "auth") {
    try {
  ...
  async function loadData(sessionUser) {
    ...
    try {
      ...
      const firstError = [v, f, p, o].find((x) => isErr(x));

      if (firstError) {
        console.error("loadData error:", firstError);

        const msg = firstError?.error?.message || "";

        if (
          firstError.status === 401 ||
          /jwt|token|expired|invalid/i.test(msg)
        ) {
        ...
        alert(firstError.error.message || "데이터 불러오기 실패");
        return;
      }
      ...
      setProducts(
        Array.isArray(p)
          ? p.map((x) => ({
              ...x,
              factoryId: x.factory_id || x.factoryId || "",
              factoryTel: x.factory_tel || x.factoryTel || "",
              colors: x.colors || [],
              bom: x.bom || [],
            }))
          : []
    ...
    return (
      ...
      >
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: 28, fontWeight: 900, color: C.acc, marginBottom: 12 }}>
            D-Works
          </div>
          <div
            style={{
    ...
    return (
      ...
      >
        <div style={{ textAlign: "center" }}>
          ...
          <div style={{ color: C.sub, fontSize: 13, marginBottom: 16 }}>데이터 불러오는 중...</div>
          <div
            style={{
              width: 32,
              height: 32,
          ...
          >
            <div
              style={{
                ...
                height: 28,
                borderRadius: 9,
                background: page === t.k ? C.acc + "15" : "transparent",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
              }}
