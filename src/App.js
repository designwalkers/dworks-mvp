  ...
  return(
    <div style={{padding:"14px 14px 80px"}}>
      ...
      {step===2&&<>
        ...
        <div style={{marginBottom:16}}><div style={{fontWeight:700,fontSize:13,marginBottom:8,color:C.txt}}>📝 전달사항 (선택)</div><textarea value={memo} onChange={e=>setMemo(e.target.value)} placeholder="전달사항 입력" style={{width:"100%",padding:"12px",border:`1px solid ${C.bdr}`,borderRadius:8,fontSize:13,fontFamily:C.fn,outline:"none",resize:"vertical",minHeight:"80px",boxSizing:"border-box"}}/></div>
...
function ProdsPage({products,setProducts,vendors,factories,user}){
  ...
  function openEdit(p){setF({...p,colors:[...(p.colors||[])],colorBom:{...(p.colorBom||{})},imageUrl:p.imageUrl||""});setSheet(true);}
  ...
  return (
    <div style={{padding:"14px 14px 80px"}}>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}><div style={{fontWeight:900,fontSize:20}}>상품 관리</div><Btn ch="+ 추가" sz="s" onClick={openAdd}/></div>
      {filtered.map(p=><Card key={p.id} onClick={()=>openEdit(p)} st={{marginBottom:10}}><div style={{fontWeight:800}}>{p.name} <Tag ch={p.season}/></div><div style={{fontSize:11,color:C.sub,marginTop:4}}>{(p.colors||[]).join(", ")}</div></Card>)}
      {sheet && <Sheet title="상품 관리" onClose={()=>setSheet(false)}><Field label="상품명" req><TxtInp val={f.name} onChange={sf("name")}/></Field><Btn ch="저장" full onClick={save}/></Sheet>}
    </div>
  );
}
...
function ListPage({orders,setOrders,products,user,onNav}){
  ...
  const filtered=orders.filter(o=>filter==="전체"||o.status===filter).sort((a,b)=>new Date(b.ts||0)-new Date(a.ts||0));
  return(
    <div style={{padding:"14px 14px 80px"}}>
      <div style={{fontWeight:900,fontSize:20,marginBottom:12}}>발주 리스트</div>
      {filtered.map(o=><Card key={o.id} st={{marginBottom:10}}><div style={{fontWeight:700}}>{o.date} 발주 <Tag ch={o.status}/></div></Card>)}
    </div>
  );
}
...
function VendorPage({vendors,setVendors,user}){
  ...
  async function save(){if(user?.token){await DB.insert(user.token,"vendors",{...f,user_id:user.id});setSheet(false);}}
  return(
    <div style={{padding:"14px 14px 80px"}}>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:14}}><div style={{fontWeight:900,fontSize:20}}>거래처 관리</div><Btn ch="+ 추가" sz="s" onClick={()=>setSheet(true)}/></div>
      {vendors.map(v=><Card key={v.id} st={{marginBottom:10}}><div style={{fontWeight:800}}>{v.name} <Tag ch={v.type}/></div></Card>)}
      {sheet && <Sheet title="거래처 등록" onClose={()=>setSheet(false)}><Field label="거래처명" req><TxtInp val={f.name} onChange={v=>setF(p=>({...p,name:v}))}/></Field><Btn ch="저장" full onClick={save}/></Sheet>}
    </div>
  );
}
...
function SettingsPage({user,setUser,factories,setFactories,onLogout}){
  ...
  async function save(){if(user?.token)await DB.updateUser(user.token,{data:pf});setUser(pf);alert("저장됨");}
  return(
    <div style={{padding:"14px 14px 80px"}}>
      <div style={{fontWeight:900,fontSize:20,marginBottom:18}}>환경설정</div>
      <Card><div style={{fontWeight:800}}>{user.name}</div><Divider/><Btn ch="로그아웃" v="w" full st={{color:C.red}} onClick={onLogout}/></Card>
    </div>
  );
}
...
export default function App(){
  ...
  async function handleLogin(u){localStorage.setItem("dworks_session",JSON.stringify(u));setUser(u);setScreen("app");loadData(u.token);}
  async function handleLogout(){localStorage.removeItem("dworks_session");setUser(null);setScreen("auth");}
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:C.fn,maxWidth:480,margin:"0 auto",position:"relative",boxShadow:"0 0 40px rgba(0,0,0,0.1)"}}>
      {/* 🚀 상단 바 수정: 브랜드명이 있으면 브랜드명, 없으면 성함 표시 🚀 */}
      ...
      <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:480,background:"#fff",borderTop:`1px solid ${C.bdr}`,display:"flex",zIndex:50, height:65}}>
        {TABS.map(t=><button key={t.k} onClick={()=>setPage(t.k)} style={{
          flex:1, background:page===t.k?C.acc+"08":"none", border:"none", borderRight:t.k!=="settings"?`1px solid ${C.bdr}`:"none", color:page===t.k?C.acc:C.sub2, cursor:"pointer", fontFamily:C.fn, display:"flex", alignItems:"center", justifyContent:"center", padding:"0 4px"
        }}>
          <span style={{fontSize:12, fontWeight:page===t.k?800:600, textAlign:"center"}}>{t.l}</span>
        </button>)}
      </div>
    </div>
  );
