...
const Btn=({ch,onClick,v="p",full,disabled,st={}})=>{
  ...
  return <button onClick={onClick} disabled={disabled} style={{background:disabled?"#EDF0F5":bg,color:disabled?"#B0B8C4":cl,border:v==="w"?`1.5px solid ${C.bdr}`:"none",borderRadius:12,padding:"14px 0",fontSize:14,fontWeight:700,cursor:"pointer",width:full?"100%":"auto",fontFamily:C.fn,display:"flex",alignItems:"center",justifyContent:"center",...st}}>{ch}</button>;
};
const Card=({children,st={},onClick})=><div onClick={onClick} style={{background:"#fff",borderRadius:16,border:`1px solid ${C.bdr}`,padding:18,boxSizing:"border-box",boxShadow:"0 2px 8px rgba(0,0,0,0.03)",...st}}>{children}</div>;
  ...
  return(
    <div style={{padding:16}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:20}}>
        ...
        <Card st={{textAlign:"center",padding:24}}><div style={{fontSize:12,color:C.sub,marginBottom:8}}>전체 발주</div><div style={{fontSize:24,fontWeight:900,color:C.ok}}>{orders.length}건</div></Card>
      </div>
    </div>
  );
}
...
function VendorPage({vendors,setVendors,user}){
  ...
  async function save(){
    ...
    const newRecord = Array.isArray(r) ? r[0] : (r.id ? r : data);
    setVendors(prev => [...prev, newRecord]);
    setSheet(false); setF({name:"",tel:"",type:"원단"});
  }
  return(
    <div style={{padding:16}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}><div style={{fontSize:22,fontWeight:900}}>거래처 관리
