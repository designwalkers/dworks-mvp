...
function DashPage({orders=[], products=[]}){
  ...
  return(
    <div style={{padding:"14px 16px 100px"}}>
      ...
      <Card>
        <div style={{fontWeight:800,marginBottom:14,fontSize:15}}>⚠️ 지연 확인 필요</div>
        {delayed.length===0 ? <div style={{textAlign:'center',padding:'20px 0',color:C.sub,fontSize:13}}>지연된 발주가 없습니다.</div> : 
          delayed.flatMap(o=>(o.items||[]).map((it,idx)=>(<div key={idx} style={{display:'flex',justifyContent:'space-between',padding:'10px 0',borderBottom:`1px solid ${C.bg}`}}><span>{products.find(x=>x.id===it.pid)?.name || "상품"} ({it.color})</span><span style={{fontWeight:800,color:C.red}}>{fmtN(it.qty)}장</span></div>)))
        }
      </Card>
    </div>
  );
...
function ProdsPage({products=[],setProducts,vendors=[],factories=[],user}){
  ...
  async function save(){
    ...
    const sd={name:f.name, category:f.category, factory_id:f.factoryId, colors:f.colors, color_bom:f.colorBom, user_id:user.id};
    try {
      let res;
      if(f.id) res = await DB.update(user.token,"products",f.id,sd);
      else res = await DB.insert(user.token,"products",sd);
      
      if(res && !res.error) {
        const item = Array.isArray(res) ? res[0] : res;
        if(f.id) setProducts(ps=>ps.map(p=>p.id===f.id?item:p));
        else setProducts(ps=>[...ps, item]);
        setSheet(false);
      } else alert("저장 실패. 데이터를 확인하세요.");
    } catch(e) { alert("연결 오류"); }
  }
  ...
  return(
    <div style={{padding:"20px 16px 100px"}}>
      ...
      {products.map(p=><Card key={p.id} onClick={()=>{setF(p);setStep(0);setSheet(true);}} st={{padding:24}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><div style={{fontWeight:800,fontSize:17}}>{p.name} <Tag ch={p.category}/></div><Tag ch={p.season||"26SS"}/></div></Card>)}
      {sheet && <Sheet title={f.id?"상품 수정":"새 상품 등록"} onClose={()=>setSheet(false)}>
        ...
        {step===0 ? <>
          ...
          <Btn ch="다음: 원부자재 등록" full onClick={()=>{if(!f.colors.length)return alert("색상을 추가하세요");setSelColor(f.colors[0]);setStep(1);}}/>
        </> : <>
